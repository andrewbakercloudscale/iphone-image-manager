...
/// Every line this helper writes to stdout is one JSON object.
/// Diagnostics go to stderr so a caller can pipe stdout straight into a parser.
enum Out {
    ...
    static func log(_ message: String) {
        lock.lock()
        FileHandle.standardError.write("[iimhelper] \(message)\n".data(using: .utf8)!)
        lock.unlock()
    }
